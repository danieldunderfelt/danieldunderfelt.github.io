---
layout: post
title: "Getting rid of Ruby (for Sass compilation)"
categories:
    - frontend
tags:
    - ruby
...
Ah, Bower. Until yesterday, I wasn't a Bower user. I saw no point in it as NPM handled everything I needed. But turns out it's pretty handy for Sass libraries!
...
Install what we can using Bower:
...
[Gulp-sass](https://github.com/dlmanning/gulp-sass) is a wrapper around [Node-sass](https://github.com/sass/node-sass) which in turn provides bindings to Libsass from Node.js. It is the easiest way to get started using Libsass. [Autoprefixer](https://github.com/postcss/autoprefixer) (with [gulp-autoprefixer](https://github.com/sindresorhus/gulp-autoprefixer)) is a tool that automatically inserts vendor prefixes where needed. If you've been relying on Compass mixins for this, Autoprefixer will be a breath of fresh air. Finally, (Gulp-sourcemaps)[https://github.com/floridoo/gulp-sourcemaps] generates sourcemaps for easy debugging in the browser. As you can see, these three Gulp plugins provide much of the basic Compass functionality. I recommend reading each plugin's readme for information about options and configuration.

Cool! Now create a `gulpfile` and use the following to start it off:
...
I always pass in `errLogToConsole: true` to the Sass plugin, because who wants the watch-loop to halt on errors? Not me! That option makes errors display in your terminal instead, without stopping the watch process.
